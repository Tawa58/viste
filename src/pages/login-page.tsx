...
export function LoginPage() {
  ...
  async function onSubmit(values: LoginValues) {
    ...
    try {
      ...
      toast.success('Welcome back to Viste High School')
      navigate('/dashboard')
    } catch (err) {
      ...
      <div className="relative mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-4 py-10 lg:grid-cols-2">
        <FadeIn className="hidden lg:block">
          ...
          <div className="mt-8 flex items-center gap-3 rounded-2xl border border-border/80 bg-card/80 p-4 shadow-card">
            ...
            <div>
              <p className="text-sm font-semibold">Secure mock authentication</p>
              <p className="text-xs text-muted-foreground">
                Development credentials only — Spring Boot auth comes later.
              </p>
            </div>
          </div>
        </FadeIn>
